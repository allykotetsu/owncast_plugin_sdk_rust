...
impl<'a> Into<Plugin<'a>> for PluginBuilder<'a> {
    fn into(self) -> Plugin<'a> {
        ...
        if !self.on_chat_message_.is_empty() { notify.push(Notify { event: EventType::ChatMessageReceived }); }
        if !self.on_chat_user_joined_.is_empty() { notify.push(Notify { event: EventType::ChatUserJoined }); }
        if !self.on_chat_user_parted_.is_empty() { notify.push(Notify { event: EventType::ChatUserParted }); }
        if !self.on_chat_user_renamed_.is_empty() { notify.push(Notify { event: EventType::ChatUserRenamed }); }
        if !self.on_message_moderated_.is_empty() { notify.push(Notify { event: EventType::ChatMessageModerated }); }

        if !self.on_stream_started_.is_empty() { notify.push(Notify { event: EventType::StreamStarted }); }
        if !self.on_stream_stopped_.is_empty() { notify.push(Notify { event: EventType::StreamStopped }); }
        if !self.on_stream_title_changed_.is_empty() { notify.push(Notify { event: EventType::StreamTitleChanged }); }

        if !self.on_sse_connect_.is_empty() { notify.push(Notify { event: EventType::SseConnect }); }
        if !self.on_sse_disconnect_.is_empty() { notify.push(Notify { event: EventType::SseDisconnect }); }

        if !self.on_tick_.is_empty() { notify.push(Notify { event: EventType::Tick }); }

        if !self.on_fediverse_.is_empty() { notify.push(Notify { event: EventType::FediverseActivity }); }
        if !self.on_fediverse_follow_.is_empty() { notify.push(Notify { event: EventType::FediverseFollow }); }
        if !self.on_fediverse_like_.is_empty() { notify.push(Notify { event: EventType::FediverseLike }); }
        if !self.on_fediverse_repost_.is_empty() { notify.push(Notify { event: EventType::FediverseRepost }); }
        if !self.on_fediverse_quote_.is_empty() { notify.push(Notify { event: EventType::FediverseQuote }); }
        ...
        if let Some(&(priority, _)) = filter_chat_message_.first() {
            filter.push(Filter {
                event: EventType::ChatMessageReceived,
                priority
            });
        }
